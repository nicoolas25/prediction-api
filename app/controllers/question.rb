  ...
  class Question < Grape::API
    ...
    namespace :questions do
      namespace ':locale' do
        ...
        get 'global/open' do
          questions = Domain::Question.global.open.for(player).ordered.with_locale(@locale).with_tags.all
        ...
        get 'global/answered' do
          questions = Domain::Question.global.open.answered_by(player).ordered.with_locale(@locale).with_tags.all
        ...
        get 'global/outdated' do
          questions = Domain::Question.global.expired.answered_by(player).ordered(:desc).with_locale(@locale).with_tags.all
          question_ids = questions.map(&:id)
          friend_service = Domain::Services::FriendQuestion.new(player, question_ids)
          winning_service = Domain::Services::Winning.new(player, question_ids)
          sharing_service = Domain::Services::SharingQuestion.new(player, question_ids)
        ...
        get 'friends/open' do
          questions = Domain::Question.global.open.for(player).answered_by_friends(player).ordered.with_locale(@locale).with_tags.all
          friend_service = Domain::Services::FriendQuestion.new(player, questions.map(&:id))
          present questions,
            with: Entities::Question,
            locale: @locale,
        ...
        get 'friends/open/tags/:tag_ids' do
          tags = Domain::Tag.matching_ids(params[:tag_ids].split(',').uniq)
          questions = Domain::Question.global.open.for(player).answered_by_friends(player).ordered.with_locale(@locale).tagged_with(tags).with_tags.all
          friend_service = Domain::Services::FriendQuestion.new(player, questions.map(&:id))
          present questions,
            with: Entities::Question,
            locale: @locale,
