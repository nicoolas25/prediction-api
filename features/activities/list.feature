Feature: Display the activity feed
  ...
  Scenario: There is some activities in the feed
    ...
    And the user "nickname" have the following "facebook" friends:
      | friend_1 |
    And it is currently 1 minute from now
    ...
    Then the response status should be "200"
    And the JSON response should have 5 "$.[*].*"
    ...
    And the JSON response should have "$.[2].question.id" with the text "2"
    And the JSON response should have "$.[2].question.made_prediction" with the text "false"
    And the JSON response should have "$.[3].kind" with the text "badge"
    And the JSON response should have "$.[3].level" with the text "1"
    And the JSON response should have "$.[4].kind" with the text "friend"
    And the JSON response should have "$.[4].players[0].nickname" with the text "friend_1"
    And the JSON response should have 5 "$.[*].players[*].social"
  ...
  Scenario: The looses aren't displayed
    ...
    And the JSON response should have "$.[2].players[*].nickname" with the text "friend_1"
    And the JSON response should have 3 "$.[*].players[*].social"
